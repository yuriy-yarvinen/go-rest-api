...
// Repository implements events.EventRepository backed by SQLite.
...
func (r *Repository) Create(event *events.Event) error {
	const query = "INSERT INTO events (name, description, location, date_time, user_id) VALUES (?, ?, ?, ?, ?)"
...
func (r *Repository) GetByID(id int64) (*events.Event, error) {
	row := r.db.QueryRow("SELECT id, name, description, location, date_time, user_id FROM events WHERE id = ?", id)
...
func (r *Repository) Update(event *events.Event) error {
	const query = "UPDATE events SET name = ?, description = ?, location = ?, date_time = ?, user_id = ? WHERE id = ?"
	stmt, err := r.db.Prepare(query)
	if err != nil {
		return err
	}
